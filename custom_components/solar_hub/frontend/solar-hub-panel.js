class SolarHubPanel extends HTMLElement {
  ...
  _battery() {
    ...
    const deepByIndex = new Map(deep.map(item => [item.index, item]));
    const batterySections = batteries.map((battery, arrayIndex) => {
      const index = battery.index || arrayIndex + 1;
      const detail = deepByIndex.get(index) || {};
      const cells = (detail.cells || []).map(cell =>
        `<div class="cell"><span>Cell ${this._escape(cell.index)}</span><strong><i class="cell-dot ${this._escape(cell.status || "unknown")}"></i>${this._escape(cell.voltage)} V</strong></div>`
      ).join("");
      const metadata = [
        ["Serial", detail.serial || battery.serial],
        ["State of charge", this._metric(battery.soc)],
        ["Live voltage", this._metric(battery.voltage)],
        ["Live current", this._metric(battery.current)],
        ["Cell temperature", `${this._metric(battery.temperature_min)} to ${this._metric(battery.temperature_max)}`],
        ["BMS temperature", detail.bms_temperature == null ? "—" : `${this._escape(detail.bms_temperature)} °C`],
        ["Design capacity", detail.capacity_ah == null ? "—" : `${this._escape(detail.capacity_ah)} Ah`],
        ["Remaining capacity", detail.capacity_remaining_ah == null ? "—" : `${this._escape(detail.capacity_remaining_ah)} Ah`],
        ["Cycles", detail.cycles ?? "—"],
        ["BMS firmware", detail.bms_firmware ?? "—"],
        ["Lifetime charge", detail.lifetime_charge_kwh == null ? "—" : `${this._escape(detail.lifetime_charge_kwh)} kWh`],
        ["Lifetime discharge", detail.lifetime_discharge_kwh == null ? "—" : `${this._escape(detail.lifetime_discharge_kwh)} kWh`],
      ].map(([label, value]) => `<div class="battery-fact"><span>${label}</span><strong>${value}</strong></div>`).join("");
      return `<section class="battery-section"><div class="battery-heading"><div><span class="kicker">DETECTED BATTERY</span><h2>Battery ${this._escape(index)}</h2></div><small>${this._escape(detail.serial || battery.serial || "Serial unavailable")}</small></div><div class="battery-facts">${metadata}</div>${cells ? `<h3>Battery ${index} cells</h3><div class="cell-grid">${cells}</div>` : `<p class="empty-cells">Run the deep Modbus scan to load this battery’s individual cell voltages.</p>`}</section>`;
    }).join("");
    return this._page("Battery", "Battery bank status and detected modules.",
      ...
      `<div class="battery-sections">${batterySections || "<p>No batteries were detected.</p>"}</div>`);
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      *{box-sizing:border-box}.shell{max-width:1500px;margin:0 auto;padding:20px}
      ...
      .stats{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-top:14px}.detail{grid-template-columns:repeat(4,1fr)}.stat,.module,.rows,.plan,.battery-section{background:#0b1815;border:1px solid #1b302b;border-radius:18px;padding:18px}.stat span{display:block;color:#819a92;font-size:12px;font-weight:800}.stat strong{display:block;font-size:22px;margin:14px 0 4px}.stat small,.plan small{color:#617a72}.stat button{margin-top:12px;background:#1b5a42;border:0;border-radius:8px;color:white;padding:9px;font-weight:800;cursor:pointer}.stat button:disabled{opacity:.55;cursor:wait}.scan-progress{height:9px;background:#122923;border:1px solid #25463b;border-radius:999px;overflow:hidden;margin:13px 0 6px}.scan-progress i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#3bbd79,#78ebb0);transition:width .45s ease}.battery-sections{grid-column:1/-1;display:grid;gap:14px}.battery-heading{display:flex;justify-content:space-between;gap:14px;align-items:start;border-bottom:1px solid #1b302b;padding-bottom:13px}.battery-heading h2{margin:5px 0 0;font-size:25px}.battery-heading small{color:#79948b}.battery-facts{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:14px 0}.battery-fact{background:#0e201b;border-radius:10px;padding:10px}.battery-fact span,.cell span{display:block;font-size:11px;color:#819a92;font-weight:800}.battery-fact strong{display:block;margin-top:5px}.battery-section h3{font-size:15px;margin:3px 0 10px}.cell-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:8px}.cell{padding:9px;background:#0e201b;border:1px solid #1a352d;border-radius:9px}.cell strong{display:flex;align-items:center;gap:6px;font-size:13px;margin-top:5px}.cell-dot{display:inline-block;width:10px;height:10px;border-radius:50%;background:#6c7e78;flex:none}.cell-dot.good{background:#55d58a;box-shadow:0 0 9px #55d58a88}.cell-dot.warning{background:#e8b84d;box-shadow:0 0 9px #e8b84d88}.cell-dot.critical{background:#e35d5d;box-shadow:0 0 9px #e35d5d88}.empty-cells{color:#819a92;margin:14px 0 0}.module{display:flex;justify-content:space-between;gap:20px}.module span{color:#829b94}.rows{margin-top:14px}.row{display:flex;justify-content:space-between;gap:18px;padding:12px 0;border-top:1px solid #182a26}.row:first-child{border-top:0}.row span strong,.row span small{display:block}.row span small{color:#5f7770;font-size:10px;margin-top:3px}.plan{margin-top:14px}.plan>strong{display:block;font-size:19px;margin:7px 0}.plan-track{position:relative;min-height:45px;display:flex;gap:5px;align-items:stretch;padding:9px 0}.plan-segment{flex:1;min-width:56px;padding:7px;border-radius:7px;font-size:11px;font-weight:800;color:#07100c;background:#777}.plan-segment.charge,.plan-segment.solar{background:#55c985}.plan-segment.discharge{background:#67a7e7}.plan-segment.import{background:#e5ad52}.plan-segment.export{background:#9c7ae6}.now-marker{position:absolute;top:0;bottom:0;left:0;border-left:3px solid white}
      @media(max-width:1050px){.stats{grid-template-columns:repeat(3,1fr)}.battery-facts{grid-template-columns:repeat(3,1fr)}.cell-grid{grid-template-columns:repeat(6,1fr)}.flow{grid-template-columns:1fr 30px 1fr 30px 1fr 30px 1fr;padding:16px}.node strong{font-size:20px}}
      @media(max-width:720px){.shell{padding:10px}header{flex-direction:column;align-items:flex-start}nav{width:100%}.heading h1{font-size:32px}.flow{grid-template-columns:1fr 1fr;gap:10px}.connector{display:none}.stats,.detail{grid-template-columns:repeat(2,1fr)}.battery-facts{grid-template-columns:repeat(2,1fr)}.cell-grid{grid-template-columns:repeat(3,1fr)}.battery-heading{flex-direction:column}}
